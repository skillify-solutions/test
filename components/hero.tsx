        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-12">
            {(kickerTitle || kickerSubtitle) && (
              <div className="mb-8 text-xs text-white/90 animate-fade-in uppercase tracking-wider">
                <div className="opacity-80">hosted by</div>
                <div className="font-semibold text-white">{kickerSubtitle ?? kickerTitle}</div>
              </div>
            )}
            <h1 className="font-heading text-[28px] sm:text-[36px] md:text-[40px] lg:text-[44px] font-bold leading-tight tracking-wide text-white drop-shadow-lg animate-slide-up">
              {title}
            </h1>
            {subtitle ? (
              <p className="mt-6 text-[15px] leading-7 text-white/95 max-w-prose drop-shadow-md animate-fade-in-delay">{subtitle}</p>
            ) : null}
